...
import { GlyphChannel, perceive, parseGlyphResponse, tilde, decayShear, type GlyphPacket, type Contradiction } from '../src/council/aukoraFuGlyph';
...
describe('aukoraFuGlyph: parseGlyphResponse (fail-closed parser)', () => {
  ...
  // Round 2 (issue #22) regression: the prompt's own instructions/examples (below, ~line 340)
...
// core/tests/fixtures/fusion-replies/). Two genuinely compliant, one real parser bug (DeepSeek's
...
describe('aukoraFuGlyph: pure canonical boundary', () => {
  it('has no network, environment, filesystem, capture, custody, or Symbiote dependency', () => {
    const src = fs.readFileSync(path.join(__dirname, '..', 'src', 'council', 'aukoraFuGlyph.ts'), 'utf-8');
  ...
  it('is consumed only by the canonical council', () => {
    const srcDir = path.join(__dirname, '..', 'src', 'council');
