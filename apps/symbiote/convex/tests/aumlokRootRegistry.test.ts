...
import { signChainHeadV3, verifyChainHeadV3, SIGNED_HEAD_V4_ALG } from "../aukoraSignedHead";
import { rotationHead, rootKeyFingerprint } from "../aumlokRootRegistry";
import { verifyReceiptChainCore, writeReceiptRow, appendIdentityLifecycleReceipt } from "../aukoraReceipts";
...
async function genesis(t: any, cav: any, rootId: string, keyId: string, publicKey: string, nonce: string, signSeed = OPERATOR_SEED) {
  ...
  return t.mutation(internal.aumlokRootRegistry.aumlokGenesisMint, { env, actualArgs, nodeId: NODE });
}
...
async function rotate(t: any, cav: any, oldRootSeed: string, statement: any, nonce: string, opts: { sigDomain?: any; sendStatement?: any; rotationSig?: string } = {}) {
  ...
  return t.mutation(internal.aumlokRootRegistry.aumlokRotateRoot, { env, actualArgs, nodeId: NODE });
}
async function revoke(t: any, cav: any, rootId: string, keyId: string, nonce: string) {
  ...
  return t.mutation(internal.aumlokRootRegistry.aumlokRevokeRoot, { env, actualArgs, nodeId: NODE });
}
...
describe("B2.1 — AUMLOK root registry: genesis (operator-PoP-gated, operator-born)", () => {
  ...
  it("the operator authorizes the SPECIFIC mint: swapping the public key after signing refuses (PoP binds actualArgs)", async () => {
    ...
    await expect(t.mutation(internal.aumlokRootRegistry.aumlokGenesisMint, { env, actualArgs: { ...signed, publicKey: otherPub }, nodeId: NODE })).rejects.toThrow("pop_req_sig_invalid");
    expect(await rows(t, "root-a")).toHaveLength(0);
  });
  it("confused-deputy refused: an operator envelope signed for a DIFFERENT method can't drive genesis", async () => {
    ...
    await expect(t.mutation(internal.aumlokRootRegistry.aumlokGenesisMint, { env, actualArgs, nodeId: NODE })).rejects.toThrow("pop_method_not_allowed");
    expect(await rows(t, "root-a")).toHaveLength(0);
  });
});
...
describe("B2.1 — id: chainKey reservation + structural absence of phrase/seed", () => {
  ...
  it("the identity registry handles ONLY public keys: no derive module, no Argon2id, no server-side seed derivation", () => {
    const src = import.meta.glob("../aumlokRootRegistry.ts", { query: "?raw", import: "default", eager: true }) as Record<string, string>;
  ...
  it("import guard: ONLY the authorized identity modules reference appendIdentityLifecycleReceipt", () => {
    ...
    // S1a: glob pinned to the flat vendored module set (tests live inside convex/ now — the donor's recursive
    // glob would swallow node_modules + this test file), and aumlokCeremony (B2.3 birth receipt) is NOT vendored,
    // so the authorized-writers list shrinks to the two vendored identity modules.
    ...
    expect(refs).toEqual(["aukoraReceipts.ts", "aumlokRootRegistry.ts"]);
  });
});
