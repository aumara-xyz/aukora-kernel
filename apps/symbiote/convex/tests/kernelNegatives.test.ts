...
import { AUKORA_DECISION_TOKEN_TTL_MS } from "../aukoraCore";

const modules = import.meta.glob(["../*.ts", "../_generated/*.js", "!../vitest.config.ts"]); // S1a: pinned glob (tests live inside convex/)

async function mint(t: any, actor: string, action: string, resource: string, ring: import("../aukoraCore").AukoraRing, sk: string) {
