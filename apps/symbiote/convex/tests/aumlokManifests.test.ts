...
import { signChainHeadV3, SIGNED_HEAD_V4_ALG } from "../aukoraSignedHead";
import { rootKeyFingerprint, rotationHead } from "../aumlokRootRegistry";
import { manifestRootHead, manifestPopHead, rootRevokeHead, subjectRevokeHead, consumeHead, resolveManifestAuthority } from "../aumlokManifests";
...
async function mint(t: any, m: any, opts: { rootSeed?: string; subjectSeed?: string; rootDomain?: any; subjectDomain?: any; rootSig?: string; subjectPopSig?: string; sendManifest?: any } = {}) {
  ...
  return t.mutation(internal.aumlokManifests.aumlokMintManifest, { manifest: opts.sendManifest ?? m, rootSig, subjectPopSig });
}
const resolve = (t: any, manifestId: string, over: any = {}) =>
  t.query(internal.aumlokManifests.aumlokManifestResolve, { manifestId, ring: "local-write", action: "echo", resource: "echo:demo", intentCodec: "json_action_v1", ...over });
...
async function consume(t: any, manifestId: string, useSeq: number, over: any = {}, subjectSeed = SUBJECT_SEED) {
  ...
  return t.mutation(internal.aumlokManifests.aumlokManifestConsume, { req: over.sendReq ?? r, subjectSig });
}
...
describe("B2.2 — revocation (root superior; subject self-revoke/pause, own manifest only)", () => {
  async function rootRevoke(t: any, manifestId: string, seed = ROOT_SEED, domain: any = "aumlokManifest") {
    ...
    return t.mutation(internal.aumlokManifests.aumlokRevokeManifest, { statement: s, rootSig });
  }
  async function selfAction(t: any, manifestId: string, action: string, seed = SUBJECT_SEED) {
    ...
    return t.mutation(internal.aumlokManifests.aumlokManifestSelfRevoke, { statement: s, subjectSig });
  }
...
describe("B2.2 — manifest receipt reservation", () => {
  it("general receipt writers cannot write mft:* (writeReceiptRow refuses the reserved prefix)", async () => {
    ...
    const { writeReceiptRow } = await import("../aukoraReceipts");
