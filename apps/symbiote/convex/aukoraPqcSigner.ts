...
export const PQC_DOMAINS = Object.freeze({
  ...
  // M2b (MINTED 2026-07-05, owner-ratified via GH #103 chat approval): dedicated erase domain — owner-root key
  // ONLY (the delegated subject can never erase; Auma's ruling). Separate from aumlokMemRecall so a captured
  // recall PoP can never replay as an erase authorization — the exact anti-rogue-key class this registry exists for.
  aumlokMemErase: "aukora-aumlok-memerase-v1",     // memory-erase proof-of-possession (owner-root key ONLY)
  // R5b (MINTED 2026-07-07, benchmark-candidate brick): dedicated SEARCH domain — owner-root key ONLY this
  // brick, ranked keys-only results. Separate from aumlokMemRecall so a captured search PoP can never replay
  // as a content read (recall) or an erase, and vice versa; the query text is inside the signed preimage.
  aumlokMemSearch: "aukora-aumlok-memsearch-v1",   // memory-search proof-of-possession (owner-root key ONLY)
  // R5d (MINTED 2026-07-09, observability brick): dedicated recent-list domain — owner-root key ONLY,
  // metadata newest-first. Separate from search/recall so a captured recent-list PoP can never replay as
  // a content read or a search query; the limit is inside the signed preimage.
  aumlokMemRecent: "aukora-aumlok-memrecent-v1",   // recent-memory proof-of-possession (owner-root key ONLY)
  // R5c CONTENDER (Great Merge round 3, #178): vector search + embedding backfill. Two dedicated
  // domains so no captured PoP can cross surfaces: a full-text search sig can never replay as a
  // vector query; an embed-backfill sig (write-adjacent but content-untouched) can never replay as
  // a memory write, recall, erase, or search — and vice versa. The query-vector hash / embedding
  // hash ride INSIDE the signed preimages.
  aumlokMemVecSearch: "aukora-aumlok-memvecsearch-v1", // vector-search proof-of-possession (owner-root key ONLY)
  aumlokMemEmbed: "aukora-aumlok-memembed-v1",     // embedding-backfill proof-of-possession (owner-root key ONLY)
