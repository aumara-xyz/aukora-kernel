...
// vitest suite, testing the actual shipped module (core/src/aukoraFuEngine.ts) rather than a re-derived
// copy of the math. No test here makes a network call — AukoraFuEngine.reason()/emitGlyph() are
// exercised nowhere in this file, matching the module's "landed dormant" status until wired.
import { describe, it, expect } from 'vitest';
import * as fs from 'fs';
import * as path from 'path';
import { GlyphChannel, perceive, parseGlyphResponse, tilde, decayShear, type GlyphPacket, type Contradiction } from '../src/aukoraFuEngine';
...
describe('aukoraFuEngine: perceiver (KL-divergence coherence)', () => {
...
describe('aukoraFuEngine: ~ operator + φ-governed shear decay', () => {
...
describe('aukoraFuEngine: phase-lock (groupthink) detection', () => {
...
describe('aukoraFuEngine: parseGlyphResponse (fail-closed parser)', () => {
...
describe('aukoraFuEngine: parseGlyphResponse against REAL captured council replies (issue #34 live run)', () => {
...
describe('aukoraFuEngine: advisory-only / evidence-never-authority discipline', () => {
  it('every Receipt-shaped literal in the module source is pinned advisoryOnly:true, grantsAuthority:false', () => {
    const src = fs.readFileSync(path.join(__dirname, '..', 'src', 'aukoraFuEngine.ts'), 'utf-8');
    const grants = src.match(/grantsAuthority:\s*(true|false)/g) ?? [];
    expect(grants.length).toBeGreaterThan(0);
    for (const g of grants) expect(g).toContain('false');
  });

  it('the module never calls the network on import, and its approved caller set is exactly {aukoraFuCouncil.ts, selfEditReviewCouncil.ts}', () => {
    // Was "nothing in this repo invokes it yet" while landed dormant (issue #15). Then wired as the
    // review engine (selfEditReviewCouncil.ts). The fusion-chat round replays PR #352's portable wrapper
    // aukoraFuCouncil.ts as a SECOND approved caller — this pin was widened NARROWLY and deliberately,
    // not silently loosened, and the no-network-on-import + advisory-only assertions are retained.
    const src = fs.readFileSync(path.join(__dirname, '..', 'src', 'aukoraFuEngine.ts'), 'utf-8');
    const codeOnly = src.split('\n').filter((l) => !l.trim().startsWith('*') && !l.trim().startsWith('//')).join('\n');
    const fetchSites = (codeOnly.match(/\bfetch\(/g) ?? []).length;
    expect(fetchSites).toBe(1); // only inside emitGlyph — no network on import
    const coreSrcDir = path.join(__dirname, '..', 'src');
    const APPROVED_CALLERS = ['aukoraFuCouncil.ts', 'selfEditReviewCouncil.ts'];
    const callers = fs.readdirSync(coreSrcDir)
      .filter((f) => f.endsWith('.ts') && f !== 'aukoraFuEngine.ts')
      .filter((f) => fs.readFileSync(path.join(coreSrcDir, f), 'utf-8').includes('aukoraFuEngine'))
      .sort();
    expect(callers).toEqual(APPROVED_CALLERS);
    // NEGATIVE: any third, unapproved caller must fail the pin.
    expect([...callers, 'rogueCaller.ts'].every((c) => APPROVED_CALLERS.includes(c))).toBe(false);
  });

  it('the CODENAME/CODename typo from the source export is fixed in actual code — only one module constant, no dead duplicate', () => {
    const src = fs.readFileSync(path.join(__dirname, '..', 'src', 'aukoraFuEngine.ts'), 'utf-8');
    const codeOnly = src.split('\n').filter((l) => !l.trim().startsWith('*') && !l.trim().startsWith('//')).join('\n');
    expect(codeOnly).not.toMatch(/\bCODename\b/); // the mixed-case duplicate must not survive in real code
    expect((codeOnly.match(/\bCODENAME\b/g) ?? []).length).toBeGreaterThanOrEqual(2); // declared + used
  });
});
